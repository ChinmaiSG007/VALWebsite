  ...
  return (
    (e = {
      638: function (e) {
        ...
        function i(e, t) {
          const duration = t.duration || 2000; // Convert duration from seconds to milliseconds
          const totalSteps = 100; // Fixed number of steps for smooth animation
          const delay = duration / totalSteps; // Calculate delay per step
          const stepValue = (t.end - t.start) / totalSteps; // Calculate value change per step
          let currentValue = t.start; // Initialize with the start value

          // Display the initial value
          e.innerHTML = u(currentValue, { ...t, element: e });

          // Set up the interval for counting
          const interval = setInterval(() => {
            currentValue += stepValue; // Increment or decrement value

            // Ensure we don’t exceed the end value
            if (
              (stepValue > 0 && currentValue >= t.end) ||
              (stepValue < 0 && currentValue <= t.end)
            ) {
              currentValue = t.end; // Set the final value
              clearInterval(interval); // Stop the animation
            }

            // Round the value to the nearest integer before displaying it
            e.innerHTML = u(Math.round(currentValue), { ...t, element: e });
          }, delay);
        }
        ...
        function u(e, t) {
          ...
          e = (function (e, t) {
            if (t.filesizing || t.currency) {
              ...
              var r = 1e3,
                n =
                  t.currency && "string" == typeof t.currency ? t.currency : "",
                o = t.decimals || 1,
                i = ["", "K", "M", "B", "T"],
                u = "";
              t.filesizing &&
          ...
          // Add + after the number if the element has the class "+"
          if (t.element && t.element.classList.contains("+")) {
            e = `${e}+`;
          }

          // Format as string with locale
          return (e = t.formater
